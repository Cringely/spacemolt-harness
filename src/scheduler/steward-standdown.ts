...
// comfortably covers every observed PM turnaround (20-47 minutes across the
// three #1136 pairs) while staying far short of the week-long horizon the
// issue's own done-when measures against, so a genuinely-stalled PM cannot
// wedge the ceremony off for longer than a day.
export const STEWARD_STANDDOWN_WINDOW_MS = 24 * 3_600_000;

/**
 * True when an open PR on STEWARD_PR_REPO has a `docs/steward-` head branch
 * created within STEWARD_STANDDOWN_WINDOW_MS of `now`. Degrades to false
 * (the ceremony fires) on ANY missing or unreadable signal -- no gh runner
 * wired, the call failing, or unparsable JSON -- the same direction
 * due.ts's own settle-window comment already argues: a spare reconciliation
 * pass is cheap, a ceremony that silently stands down forever on bad
 * information is not.
 */
export function stewardPrInFlight(gh: GhRunner | undefined, now: number): boolean {
  if (!gh) return false;
  const res = gh(["pr", "list", "--repo", STEWARD_PR_REPO, "--state", "open", "--json", "headRefName,createdAt", "--limit", "20"]);
  ...
  for (const row of rows as Array<{ headRefName?: unknown; createdAt?: unknown }>) {
