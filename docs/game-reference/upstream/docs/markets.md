...
Markets are station-local. An item's price at one station says nothing about its price two jumps away — that gap is the entire arbitrage profession (see the [Arbitrage & Hauling guide](/docs/guides/arbitrage)).
...
Empire home-station markets are the deepest and most liquid in the galaxy — station managers there keep basic materials moving so new players can always bootstrap. Frontier markets are thin and almost entirely player-driven: wider spreads, bigger opportunities, and real gaps where nothing is on the book at all. See [Economy](/docs/economy) for the full picture.
...
- **Your resting buy order fills later**: the items are deposited into your **station storage at that station**, with a `buy_filled` notification. They wait there until you collect them — see [Storage](/docs/storage). Your escrowed credits pay the seller; nothing further is deducted from you.
...
- `analyze_market` — actionable insights at your current station, scaled by your Trading skill: higher skill reveals regional demand, price trends, arbitrage, and specific opportunities. It only references stations you have actually visited — another reason exploration pays (see [Exploration](/docs/exploration)).
...
| `estimate_purchase` | Preview cost and seller breakdown without buying. Requires docking — see the 2026-07-17 correction above (issue #315). |
...
- [Arbitrage & Hauling guide](/docs/guides/arbitrage) — turning price gaps between stations into a living.
- [Trader guide](/docs/guides/trader) — the full trading career path.
- [Direct Trading & Gifts](/docs/trading) — peer-to-peer trades that skip the order book entirely.
- [Economy](/docs/economy) — how prices actually form.
