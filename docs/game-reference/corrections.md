...
## Cargo weight does not affect fuel burn (`upstream/docs/travel.md`)

**Correction (2026-09-19, reference self-contradiction, issue #1045):** `travel.md` line 28 says inter-system jump fuel "scales with ship mass and speed, and mass includes what you are hauling, so a loaded freighter burns more per jump than an empty one." That sentence is an unrevised summary line predating a dated engine change recorded in the reference's own changelog: `upstream/guides/fuel.md` line 551, v0.195.0 (March 9, 2026), "Cargo weight no longer affects fuel consumption" (restated at line 571, "Cargo doesn't affect fuel (as of v0.195.0)"). `upstream/guides/arbitrage.md` line 99 agrees: "Cargo weight does *not* affect fuel burn, so a full hold flies as cheap as an empty one." The published formulas carry no mass or cargo term at all: jump cost is `ceil(shipScale^1.5 × shipSpeed × 10.0 × 0.10)` (fuel.md line 318) and intra-system travel is `ceil(shipScale^1.5 × shipSpeed × distance × 0.07)` (fuel.md line 278). Per AGENTS.md's evidence-precedence rule, the dated changelog and the published formulas win over the stale summary sentence. Treat cargo load as irrelevant to fuel burn until upstream revises travel.md; the improv-mode briefing (`docs/superpowers/specs/2026-07-12-improv-mode.md` §4) no longer claims otherwise as of this entry.
