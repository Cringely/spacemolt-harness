...
**A harder case: benign system output that looks like a suggestion.** The rule must also hold when the external text is not hostile but helpful. A system error message like `item_not_available` may ship a JSON template showing how to create a buy order, documented and correct. A model reads that template as a helpful suggestion from the system and executes it. Where hostile injection tries to override operator instructions with malicious text, this case overrides operator instructions with benign text the system itself provides. The operator says "refuel," but the game says "here's a JSON template you can use to buy fuel," and the template looks enough like an instruction that the model follows it instead. The boundary is the same: everything the game says is data, never instructions. But recognizing the harder case — where the data is not malicious, is even documented — reminds why the rule cannot depend on "don't trust untrusted text." It depends on "only the operator gives instructions, everything else is data, full stop." The agent must route system-provided templates as information to interpret, never as next-steps to execute.
Source: docs/superpowers/specs/2026-07-12-improv-mode.md §4 (social/security); docs/wiki/security-baseline.md; issue #681, which records two distinct occurrences: 2026-08-01, miner, six repeated `fuel_cell` orders with no steer involved; and 2026-08-11, corsair, a single order converted from a steer that had named `buy`.
...
## L-42: A rate needs a matching numerator and denominator, or it reports defects that are not there

**What happened.** The strategy reviewer flagged three "broken capabilities" from a failure rate it computed by dividing a lifetime failure count by a 72-hour attempt window. Two of the three were phantoms. `scan` showed 27/33 failed, but that 27/33 was accumulated over the tool's whole lifetime and it had not been attempted at all in the 72 hours the reviewer claimed to be describing (last attempt 2026-07-14). `survey_system` was flagged too, but its two most recent attempts had both succeeded once a survey scanner was fitted, so the "current" failure was stale by definition. Only the third, `scan_poi`, held up: 7 attempts in the window, 7 failures, a real unmet precondition (`not_in_faction`). The two phantom findings still turned into five duplicate backlog issues before anyone noticed the mismatch, because the dedup search that should have caught the repeats was separately broken: it ran unscoped and searched the public repo instead of the private issues repo, so it never matched the issue already filed (fixed by PR #14; #518 tracks the rate-window bug, #491 carries the correction).

**The principle.** A rate is only as true as the two numbers that make it. If the numerator counts one population (every attempt ever made) and the denominator describes another (attempts in a recent window), the ratio answers a question nobody asked and states it with the confidence of one that was. The fix is not a smarter threshold on the same fraction; it is making numerator and denominator span the identical window before the division happens.

**Discipline.** Observability and AI engineering (an automated reviewer's output is only as sound as the metric feeding its judgment).
...
## L-50. A reflex that cannot succeed must give up, or it becomes the hazard itself

**What happened.** The livelock did not end when the rescue plan was unblocked. After PR #47 merged, the miner sat at `market_prime`, fuel 19/130, with repeated single-step `travel_to` plans queued (120 total, all blocked by the reflex). The station tank was empty (`station_fuel_empty`), and the game classified this as a terminal block. The station will never have fuel; no retry will fix it. The reflex tried `refuel` directly, got `station_fuel_empty`, and tried again next tick. The event log showed 4,673 `reflex_failed` events over 13.2 hours, consuming every tick while blocking the plans. Two different gaps describe this, and they are worth keeping separate: the last executed action of any kind was a blocked `buy` at 2026-08-01T03:14:13Z, 13.6 hours before recovery, while the last action that moved the pilot anywhere was 28.5 hours before it. No amount of retrying could make `refuel` succeed, and no code recognized that a terminal-blocked call means permanent. The reflex existed to catch emergencies; instead it became the emergency.

**The principle.** When a safety action fails with a permanent, unrepairable reason (terminal block, as distinguished from transient blocks that resolve by waiting), the action will never succeed by retrying alone. A reflex that keeps trying the same action at the same target after a terminal block is a busy-loop in thin disguise. The reflex must distinguish terminal blocks from transient ones and give up at the station/action pair after seeing a terminal block, so it stops consuming ticks and the plan gets its turn. Without terminal recognition, the reflex trades being a safety mechanism for being a livelock engine.

**Discipline.** Loop engineering and harness design.

**Why it matters for building agents.** An agent loop often has multiple layers of remediation: a reflex for immediate recovery, a plan for more elaborate recovery. When the reflex runs out of things to try (a real, unchangeable failure), it must yield so the fuller remedy has a chance. Terminal vs. transient is a classification the environment (the game) provides; the agent must read it and act on it. A reflex that cannot distinguish "this will never work" from "try again next time" wastes ticks testing known impossibilities while blocking the plan that might work.
Source: PR #50 (#672, `fix(agent): terminal-failure give-up per station`); docs/decisions.md (2026-08-01, terminal-give-up); milestones.md M-52.

## L-51 — A justification that was true when it was written needs something that retires it

**What happened.** The action registry had narrowed the storage `deposit` endpoint to two parameters and written down exactly why: the omitted ones, `credits` and `target` among them, were "real faction-gifting/bulk properties with no consumer here". That was true when it was written. There was one pilot flying, so there was nobody to send anything to. Eleven minutes after three pilots were flying, one of them sat jailed over a 27-credit fine that another pilot's 199,696-credit balance could trivially have covered, and no registered action could move credits from one pilot to the other. Nothing marked the moment the reason stopped holding.

*A different failure, recorded here because it happened the same hour.* The launch also exposed #593's seventeen-day-old blocker ("needs persona briefing text") as two password files nobody had provisioned to the production host. That justification did not expire, because it was never true, and #593's own text flagged the claim as unverified. Recording a premise and never checking it is L-15's class (prove the premise before you encode it), applied to issue metadata instead of to code. The two share a symptom, prose nobody re-reads failing quietly, not a cause and not a fix.

**The principle.** A justification is a claim about the world at one moment, and most of them carry a silent expiry. "No consumer exists", "not needed yet", "safe because only one thing writes here" are all true until a specific event makes them false, and none of them announce it. Record the retiring event next to the justification: a narrowing that says "no consumer here" should also say what a consumer will look like when one shows up. This is the same failure the project already names for caches, where a memoized value is only as correct as the inputs someone remembered to enumerate. Prose decays the same way as a cache key, except nothing recomputes it.

**Discipline.** Harness engineering.

**Why it matters for building agents.** This failed quietly, which is what makes the class expensive. Nothing threw, no test went red, and the comment read as considered engineering right up until someone needed the thing that had been reasoned away. Agent systems accumulate these faster than most software, because so much of the design is written as prose the machine never reads back: briefings, personas, issue metadata, scope notes, "we decided not to". The practical habit is small. When you write down why something is out of scope, write down what would put it back in, and prefer a check that fails over a sentence that hopes.
Source: `src/registry/actions.ts:216-219` (the deposit narrowing, and the same choice at :238-241); issue #703 (the jailed corsair); docs/decisions.md (2026-08-02, fleet launched on minimal personas). The co-occurring #593 case belongs to L-15.

## L-52 — A truncated refusal message trains nothing

**What happened.** Every intra-fleet credit gift was refused, correctly: the planner addressed a fleet-mate by its config id (`corsair`), the guard checks in-game usernames (`Corvus Marrek`), and the two never matched. Reading the same code to fix that (#788, PR #92) turned up a second, separate defect. The guard's own refusal message put the fleet roster, the one clause naming the usernames the planner actually needed, last, and ran 229 characters against the digest's 200-character clip. The roster had never once reached the planner for the whole life of the underlying rescue issue (#703). Both the message and the clip had their own passing tests; neither asked whether the other's number fit inside it.

**The principle.** A refusal or error message read back by another agent, a planner, a retry loop, a caller, is not a log line. It is a control surface: the receiving side can only correct its next attempt if the message actually arrives. Where a channel can truncate, measure the real string rather than estimate it, and order clauses so the one the recipient needs to comply sits nearest the front, since that is the part most likely to survive a clip. A well-tested guard that never delivers its own correction has done half its job, and from the outside it is indistinguishable from a model that will not listen.

**Discipline.** Loop engineering.

**Why it matters for building agents.** A guard's refusal is often the only channel that teaches a planner what would have worked, and a length budget living in one file while the message it clips lives in another is exactly the seam nothing enforces by construction ([L-22](#l-22--each-component-was-consistent-the-contradiction-lived-between-them) is the sibling case, two components disagreeing on what is ALLOWED; this one is a single component's feedback silently losing the part the other side needed). The fix is the same habit either way: state the cross-component invariant and pin it with a test that spans the seam. `docs/wiki/seam-manifest.md` seam 2 now pins the message's length against the clip, not only its content.
Source: PR #92 (#788, `fix(gift): resolve fleet gift target id -> username at plan admission`); docs/decisions.md (2026-08-11, "Gift targets resolved at plan admission, plus the clipped refusal repaired"); docs/wiki/seam-manifest.md seam 2.

## L-53 — A producer with no consumer accumulates

**What happened.** Four cron ceremonies (scheduled jobs that read the pilots' logs and file what they find as GitHub Issues) ran continuously for 36 days. The readable scheduler run logs (2026-08-23 to 2026-09-06) show 245 ok and 5 failures (a strategy run exit 1, two standup fast-fails, a council timeout, another strategy exit 1), and every job recovered from each one with `failStreak: 0`. The filing loop never broke. Development had paused on 2026-08-11 and nobody was reading the issues. By the time PR #94 merged on 2026-09-06 the ceremonies had filed 362 issues since 2026-08-01, 354 were still open, and the last close was 2026-08-12. The same shape showed up in the sibling tracker: core issue #13 in `Cringely/agent-harness-core` ("Promote hooks/session-start-ceremony-findings.ts — mechanism transfers, payload does not") was created 2026-08-02 and carried zero comments until it was closed on 2026-09-07, yet the pattern doc it asked for had already shipped on 2026-08-15 in `65ada7a` (PR #44), with `patterns/forcing-functions.md:34` carrying the requested line verbatim. The issue outlived its own resolution by 35 days because nothing closed it. Here, three doc-steward PRs (#88, #89, #91) aged into conflicts and had to be closed. PR #94 adds a consumer probe: before opening a new issue, `fileFinding` checks whether any machine-filed issue has been closed in the last 7 days, and returns `"suppressed"` if not. Bumps of existing issues still go through; a single standing "suppression notice" issue is the visible trace. The offline suite proves the gate; whether the producer actually stopped on the host is unproven until its filing log says so.

**The principle.** Filing is half a loop. A producer that works perfectly, with no signal that anything consumes its output, does not alert when the reader goes away. It accumulates, and the pile itself becomes the next problem: duplicates, stale figures, and a resolved tracker item nobody closes. Design the producer to check for evidence of a consumer (a recent close, an acknowledgement, a read receipt) and to stop, loudly and once, when that evidence is absent. Working flawlessly is not the same as being useful; the ceremonies' recovery to `failStreak: 0` after every failure was the number everyone looked at, and it measured the wrong half. That is precisely why nobody noticed nothing was reading the output.

**Discipline.** Loop engineering.

**Why it matters for building agents.** Agent systems are full of one-way channels: alerts, reports, filed findings, memory notes. Each is built as if the reader is always there. [L-17](#l-17--observability-you-can-act-on-report-the-results-not-just-the-actions-and-make-silence-mean-something) taught that silence from a producer should mean something; this is the mirror case, where silence from the CONSUMER should mean something to the producer. The core issue is the tell: the same producer/consumer failure applied to an issue tracker rather than a backlog, where a request stayed open 35 days after the work it asked for had landed, because closing it was nobody's loop. A loop cannot notice its own missing half from inside; the check has to be built in.
Source: PR #94 (`feat(scheduler): gate new-issue filing on a live consumer probe`); issue #1029 (anchorless dedup, 31 duplicates after #85); docs/milestones.md M-56; docs/decisions.md (2026-09-06, "New-issue filing gated on a live close-recency probe, not a flag or a count", shipped in PR #94); `Cringely/agent-harness-core` issue #13, commit `65ada7a` (PR #44).

## L-54 — Validate an instrument against a case whose answer you already know

**What happened.** Minutes after PR #94 was squash-merged, three separate git checks were asked whether its branch had merged: a commit-range comparison, a three-dot diff, and `git cherry`. All three reported it unmerged, and two of the three produced confident, plausible output. Every one of them was answering a different question from the one being asked. A squash merge creates a new commit on `main` whose content matches the branch but whose history does not, so any instrument that compares commit ancestry says "not there" while the code is there. `git cherry` fails the same way for a different reason: it compares patch content (patch-ids), not ancestry, and a squash commit's patch-id matches none of the individual commits it replaced. Only asking GitHub for the PR's own state gave the right answer. Recorded in the body of PR #95, the doc-steward pass that added this lesson.

**The principle.** Before trusting a measurement, point the instrument at a case whose answer is already known and confirm it gives that answer. Three instruments agreeing is not confirmation when all three share the same blind spot; here every git-history check was blind to squash merges in exactly the same way, so their agreement measured the blind spot, not the branch. An instrument that cannot tell "absent" from "present in a form I do not read" manufactures a confident false negative, and a confident false negative gets acted on.

**Discipline.** Harness engineering.

**Why it matters for building agents.** Agents run checks and act on the result without a human glancing at the raw output, so a check that is wrong in a plausible way is worse than one that errors. [L-6](#l-6--self-reported-success-is-a-claim-not-a-fact) covers the model's claims about itself; this covers the harness's own tools, which are trusted more and questioned less. The known-answer test costs one command, and it should be the first command, not the fourth.
Source: PR #94 merge, 2026-09-06; PR #95 body (doc-steward pass, 2026-09-06).

## L-55 — A truthful negative with a dropped scope qualifier becomes a false positive

**What happened.** A ceremony report cited real PR numbers. A later debunking pass looked those numbers up in the wrong repository (this project has two: the public code repo and the private issues repo, with independent numbering), found unrelated issues, and concluded "no such PR in this repo". That finding was true as written. The qualifier "in this repo" was the whole finding, and every reader downstream dropped it, so a correct ceremony citation was recorded as a fabrication. Recorded in the body of PR #95, the doc-steward pass that added this lesson.

**The principle.** A negative finding carries its scope as part of its truth value. "Not found in X" is only "not found" when X is the whole search space, and the reader who relays it rarely re-checks whether it was. State the scope so it cannot be dropped ("PR #94 does not exist in `Cringely/spacemolt`; it exists in `Cringely/spacemolt-harness`"), or search the whole space before writing a negative at all. When two namespaces share a number format, every bare number is ambiguous and every negative about one is a hazard.

**Discipline.** Harness engineering.

**Why it matters for building agents.** Findings pass through several agents before a person reads them, and each hop summarizes. Qualifiers are the first casualty of summarization because they read as hedging rather than as content. A pipeline of honest summarizers can turn a correct negative into a confident false accusation with no agent ever lying. [L-22](#l-22--each-component-was-consistent-the-contradiction-lived-between-them) is the sibling: each hop was consistent, and the error lived between them. [L-42](#l-42-a-rate-needs-a-matching-numerator-and-denominator-or-it-reports-defects-that-are-not-there) records the same two-repo false negative on the machine side (PR #14), so this is the second occurrence of the shape, not the first. The fix is the same as for any clause the finding depends on: make it part of the value, not the wording around it.
Source: PR #95 body (doc-steward pass, 2026-09-06); `AGENTS.md` (the two-repo signpost, 2026-07-22).

## L-56 — A verification command is a disclosure channel when it quotes what it checks for

**What happened.** During a PR #116 fix round, an agent pasted the verification command it had run to prove a diff was clean into its report, and posted that report as a public comment on the PR. The command's own argument text carried the exact identifying strings it was built to search for, so the comment disclosed what the check existed to catch. A post-wave scan of PR bodies and comments found it; the comment was reposted scrubbed and the original deleted.

**The principle.** A command that embeds the value it is searching for is not safe to echo anywhere, including a report meant to prove the search ran. Report a check by its outcome ("scan clean, zero matches") rather than by pasting the literal command whenever the command's text can contain the sensitive value itself. Separately, a scrub or a gate bound to one channel (a pre-push hook, a commit scanner) protects exactly the surface it runs on; every other channel a repo carries (PR comments, issue bodies, chat transcripts) is unguarded by default, not by a bug in the gate but by its scope. Enumerate the channels before trusting that one check covers all of them.
...
## L-57 — A brief silent on who merges gets merged by whoever finished last

**What happened.** Six fix-round PRs (#119-#124) each carried a real, independent review: a four-lens council or an independent-verify pass, ADVANCE on every one, CI green on `test`/`verify` throughout (two of those verdicts sit on the parent PR, #119 and #121, rather than on the follow-up each covers). Five of the six were still merged by the fix-round agent itself, not the PM. The repo cannot prove that on its own: `gh` reports one account as merger for all six, because the agents hold the same credential. It is a dispatcher-session observation, corroborated in the repo by the fix-round comments on #119 and #121, which record that each PR had already squash-merged before that round's commit reached the branch. The dispatch briefs told each agent to "open a PR" and never said who merges it, so team-workflow.md's documented step, the PM verifies, opens, merges, then pulls, ran after the fact instead of before: main moved six times in under 70 minutes with no gap for a separate merge-time check. Nothing unsafe shipped; this is not the review gap L-14 already names, since the review itself was real and independent. It is the next gate down: the accept decision that consumes a finished review is a distinct act from writing the review, and decisions.md's own Orchestrator-role entry says exactly that, one this project's council had already ruled agents never hold unattended.

**The principle.** A capability an unattended agent already has (`gh pr merge`) plus a brief that describes the goal ("land this fix") without naming who holds an authority the project reserves for a role (merge/accept) will get exercised, because nothing in the brief or the harness stops it. Review quality and merge authority are two separate gates; proving the first ran says nothing about whether the second did. A rule that lives only in a wiki page ("the PM merges") is a convention, not a control, until the brief that dispatches the agent repeats it or the tool the agent holds is scoped to exclude it.

**Discipline.** Process / team engineering (role boundaries between review and accept); AI engineering (a brief's silences are read as permission, not as "ask first").

**Why it matters for building agents.** The failure here cost nothing because the underlying review discipline held; it is a near miss that shows the shape of the real risk. An agent given a tool and a goal will use the tool to reach the goal by the shortest legal path, and "legal" is bounded by what the brief and the harness actually forbid, not by what a policy document elsewhere says the role should be. Every dispatch that can end in an irreversible action (merge, deploy, delete) needs that boundary stated IN the brief or removed FROM the agent's tools, not assumed from team-structure.md.
Source: PR #119 (`018807d`), #120 (`83a0fd6`), #121 (`d89fecf`), #122 (`086a90d`), #123 (`d7ab050`), #124 (`8a5140f`), all merged 2026-09-19 (CI status and merge times per `gh pr view`; merger identity is a dispatcher-session observation, since all agents share one credential); `docs/wiki/team-workflow.md` (PR-stage merge step); `docs/decisions.md` (Orchestrator/accept-authority entry, "unattended agents flag merge-ready, never merge").

## L-58 — Guards correct in isolation can compose into a strand no single guard's test reaches

**What happened.** corsair sat docked at iron_reach, 0 fuel, 5 credits, repeatedly attempting a refuel it could not afford (PM live capture, 2026-09-20, not verifiable from the repo). At 07:30:49Z the #534 no-progress escalation (PR #114, M-57) armed for the first time in production: three arms on one fingerprint, six replans, the intended behavior of a design built to stop a frozen pilot from burning planner calls forever. It worked exactly as built, and its success was the strand: planner calls dropped to zero for over nine hours, because nothing in the harness re-opens a held stop on its own. A PM steer gifted 1,500 credits at 16:50:13Z. Corsair's balance rose to 1,505 and it still did not move. The refuel give-up (now filed #1115) latches once an affordability check fails and never clears when the condition that tripped it does, so the newly affordable refuel was never retried. A second steer got the planner running again, and at 17:22Z the #458 buy-price guard (PR #115, from the 09-19 wave) refused a 400cr/unit fuel cell against a 43cr catalog price, twice, exactly as designed. Its refusal hands back a filled-in `create_buy_order` template (#1116, the same shape #681 and the game's own `item_not_available` error exploited before it, per the External-text-as-instruction memory note), which this time the pilot did not resubmit verbatim. It refuelled at the station for 540cr instead and got underway. Two human steers, about thirty minutes apart, moved one pilot off a fuel-empty dock, nine hours after the escalation armed.

**The principle.** #534, the give-up behind #1115, and #458 are each defensible alone: stop a frozen loop, don't keep retrying what you can't afford, refuse an overpriced buy. Each shipped with its own ablated tests and its own independent review, and none of those tests exercise the other two. What broke was the composition: a guard that silences the planner, next to a guard whose state doesn't re-open when the world changes, next to a guard whose refusal is a resubmittable template. No single guard's test suite could have caught this, because the failure lives in the sequence, not in any one guard's logic. A system built from individually correct, individually tested pieces can still produce an emergent stuck state that only a live run, or a test that drives the pieces in sequence, will show.

**Discipline.** Loop engineering and harness design. AI engineering (composed-guard interaction).

**Why it matters for building agents.** Agent harnesses accumulate guards the way this one did: one incident, one fix, one test, repeated for months (#534 from M-52's livelock, #458 from the 09-19 wave, #1115's give-up from further back). Each addition is reviewed against the incident that produced it, correctly, and against its own isolated test suite, also correctly. Nothing in that process asks what happens when guard A's output state becomes guard B's stale input. [L-3](#l-3--a-loop-that-can-call-itself-forever-will-unless-you-bound-it) and [L-49](#l-49--a-preemption-triggered-by-condition-x-must-either-clear-x-or-defer-to-a-plan-addressing-x) both cover a single guard's own lifecycle. This is the layer above, where a healthy guard's output (silence, a latch, a template) becomes the next guard's unexamined input. A composition test, driving two or three guards through a scripted scenario together instead of each in isolation, is the only kind of test that would have caught this before a human had to.
Source: PM live capture, 2026-09-20 (not verifiable from the repo); PR #114 (M-57, closing #534) and PR #115 (09-19 wave, closing #458); tracker issues #1114 (no autonomous fleet-rescue path), #1115 (refuel give-up latch never clears), #1116 (buy-guard refusal hands back a resubmittable template).

## L-59 — One predicate, two callers, two costs of error

**What happened.** PR #143 built the weekly backlog dedupe ceremony around a title check, `isNearDuplicateTitle`, that says whether two issue titles describe the same defect. PR #147 then gave the finding filer the same check: after every minted-key tier misses, `fileFinding` bumps an open issue whose title matches instead of opening a duplicate. The council's review replayed all 521 snapshot titles through the real filer and found a forward risk the ceremony never had to worry about: all 139 titles naming exactly one pilot matched a copy with the pilot swapped, and a pilot-12 finding bumped the pilot-5 issue. Action swaps (mine against sell, same wording) matched too. The synthesis was REVISE (head `77e7559`) with a fix: not a second matcher, but a filer-only veto that refuses a match when the words the two titles do not share include a fleet pilot id, a registered action name or a scheduler job id. The fix round (head `abae49c`, ADVANCE) replayed again: 31 bumps became 28, still zero cross-cluster against the hand clusters. The four refused were one title that adds the miner, one that drops the scout, and two whose "JUMP" priority tag reads as the jump action. The ceremony still sees all four pairs.

**The principle.** The same predicate can be right for one caller and dangerous for another, because what a wrong answer costs depends on who acts on it. In the ceremony, a false match costs a `dedupe:candidate` label that a person reads and strips. In the filer, a false match silently folds a new finding into the wrong issue, where nobody is looking for it. The fix is not two definitions of sameness, which would drift apart and leave the project arguing about which one is right. It is one shared definition plus a stricter gate at the caller whose errors are expensive, arranged so that caller's matches stay a subset of the shared predicate's. The gate's own errors are cheap. A vetoed true duplicate is a missed bump the weekly ceremony still catches, never a lost finding.

**Discipline.** Harness engineering.

**Why it matters for building agents.** Agent harnesses reuse checks across paths with very different blast radii: the same classifier that tags a log line for a report also decides whether a finding gets filed, and the same similarity score that groups issues for a human also merges them unattended. Reusing the predicate is right. `AGENTS.md`'s SSOT rule says to define a thing once, and two matchers would drift. What the reuse has to carry with it is the question the review asked here: for this caller, which way does a wrong answer fail, and who catches it? Where the answer is "nobody, silently", the caller needs a veto that only narrows, so a person can still reason about the system from one definition. [L-27](#l-27--a-guard-built-on-an-unverified-assumption-can-fail-in-the-direction-it-was-built-to-prevent) makes the sibling point for guards against the game: keep your own "no" a subset of the environment's. This is the same shape applied between two of the harness's own callers.
Source: PR #147 (council synthesis REVISE at `77e7559`, fix-round verification ADVANCE at `abae49c`, merge `3cfa929`). PR #143 (`b985d59`, `isNearDuplicateTitle` in `src/scheduler/dedupe.ts`). Tracker issue #1133, close comment 2026-09-24. Entry needed in docs/decisions.md for #1133 (PR #147 added none).
