...
**What happened.** Two instances in one day, both surfacing within an hour of the same fleet launch. Issue #593 had carried "needs persona briefing text (#159)" as its blocker for seventeen days. The epic tracking it had already examined #159 and rejected it as this gate's dependency, and the real obstacle was two password files that existed in the workstation's `secrets/` and had never been copied to the production host. Nobody checked the recorded blocker against the live host config; the project simply planned around it. Separately, the action registry had narrowed a storage endpoint to two parameters and written down exactly why: the omitted properties were "real faction-gifting/bulk properties with no consumer here". That was correct at the time, when there was one pilot and so nobody to send anything to. Eleven minutes after three pilots were flying, one of them sat jailed over a 27-credit fine that another pilot's 199,696-credit balance could trivially have covered, and the harness had no way to say so.

**The principle.** A justification is a claim about the world at one moment, and most of them carry a silent expiry. "No consumer exists", "blocked on X", "not needed yet", "safe because only one thing writes here" are all true until a specific event makes them false, and none of them announce it. Record the retiring event next to the justification: a narrowing that says "no consumer here" should also say what a consumer will look like when one shows up. This is the same failure the project already names for caches, where a memoized value is only as correct as the inputs someone remembered to enumerate. Prose decays the same way as a cache key, except nothing recomputes it.

**Discipline.** Harness engineering.

**Why it matters for building agents.** Both of these failed quietly, which is what makes the class expensive. Neither threw an error, no test went red, and both artifacts read as considered engineering right up until someone needed the thing that had been reasoned away. Agent systems accumulate these faster than most software, because so much of the design is written as prose the machine never reads back: briefings, personas, issue metadata, scope notes, "we decided not to". The practical habit is small. When you write down why something is out of scope, write down what would put it back in, and prefer a check that fails over a sentence that hopes.
Source: docs/decisions.md (2026-08-02, fleet launched on minimal personas; issues #593, #703).
