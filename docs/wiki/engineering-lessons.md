...
**What happened.** Four cron ceremonies (scheduled jobs that read the pilots' logs and file what they find as GitHub Issues) ran without a single failure for 36 days. Development had paused on 2026-08-11 and nobody was reading the issues. By the time PR #94 merged on 2026-09-06 the ceremonies had filed 362 issues since 2026-08-01, 354 were still open, and the last close was 2026-08-12. The same queue held a core recommendation of the fix itself, unread for a month, and three doc-steward PRs (#88, #89, #91) that aged into conflicts and had to be closed. PR #94 adds a consumer probe: before opening a new issue, `fileFinding` checks whether any machine-filed issue has been closed in the last 7 days, and returns `"suppressed"` if not. Bumps of existing issues still go through; a single standing "suppression notice" issue is the visible trace. The offline suite proves the gate; whether the producer actually stopped on the host is unproven until its filing log says so.

**The principle.** Filing is half a loop. A producer that works perfectly, with no signal that anything consumes its output, does not alert when the reader goes away. It accumulates, and the pile itself becomes the next problem: duplicates, stale figures, and a fix recommendation buried among 350 siblings. Design the producer to check for evidence of a consumer (a recent close, an acknowledgement, a read receipt) and to stop, loudly and once, when that evidence is absent. Working flawlessly is not the same as being useful; the ceremonies' clean `failStreak: 0` was the number everyone looked at, and it measured the wrong half.

**Discipline.** Loop engineering.

**Why it matters for building agents.** Agent systems are full of one-way channels: alerts, reports, filed findings, memory notes. Each is built as if the reader is always there. [L-17](#l-17--observability-you-can-act-on-report-the-results-not-just-the-actions-and-make-silence-mean-something) taught that silence from a producer should mean something; this is the mirror case, where silence from the CONSUMER should mean something to the producer. The recursion here is the tell: the design that lacked a consumer check produced, as one of its outputs, the recommendation to add one, and that output sat unread in the same queue. A loop cannot notice its own missing half from inside; the check has to be built in.
Source: PR #94 (`feat(scheduler): gate new-issue filing on a live consumer probe`); issue #1029 (anchorless dedup, 31 duplicates after #85); docs/milestones.md M-56. Entry needed in docs/decisions.md, see PR #94.

## L-54 — Validate an instrument against a case whose answer you already know

**What happened.** Minutes after PR #94 was squash-merged, three separate git checks were asked whether its branch had merged: a commit-range comparison, a three-dot diff, and `git cherry`. All three reported it unmerged, and two of the three produced confident, plausible output. Every one of them was answering a different question from the one being asked. A squash merge creates a new commit on `main` whose content matches the branch but whose history does not, so any instrument that compares commit ancestry says "not there" while the code is there. Only asking GitHub for the PR's own state gave the right answer. Recorded in the PR body of the doc-steward pass that added this lesson (branch `docs/steward-2026-09-06-consumer-gate`).
...
**Why it matters for building agents.** Findings pass through several agents before a person reads them, and each hop summarises. Qualifiers are the first casualty of summarisation because they read as hedging rather than as content. A pipeline of honest summarisers can turn a correct negative into a confident false accusation with no agent ever lying. [L-22](#l-22--each-component-was-consistent-the-contradiction-lived-between-them) is the sibling: each hop was consistent, and the error lived between them. The fix is the same as for any clause the finding depends on: make it part of the value, not the wording around it.
Source: doc-steward PR body for branch `docs/steward-2026-09-06-consumer-gate`; `AGENTS.md` (the two-repo signpost, 2026-07-22).
