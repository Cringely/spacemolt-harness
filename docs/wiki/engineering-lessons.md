...
**What happened.** The livelock did not end when the rescue plan was unblocked. After PR #47 merged, the miner sat at `market_prime`, fuel 19/130, with plan `[travel_to Haven, dock]` queued. The station tank was empty (`station_fuel_empty`), and the game classified this as a terminal block. The station will never have fuel; no retry will fix it. The reflex tried `refuel` directly, got `station_fuel_empty`, and tried again next tick. It failed every tick for the same reason for 28 hours. The rescue plan never ran because the reflex consumed every tick on a call that terminal-blocked. No amount of retrying could make it succeed, and no code recognized that a terminal-blocked call means permanent. The reflex existed to catch emergencies; instead it became the emergency.
