...
Source: docs/superpowers/specs/2026-07-12-improv-mode.md §4 (social/security); docs/wiki/security-baseline.md; issue #681 (2026-08-02, corsair repeated buy orders from game-provided template).
...
**Why it matters for building agents.** A feature that passes automated tests in isolation can break in production the moment it ships alongside another recent feature, because the features' interaction only appears when both are deployed together. As a team parallelizes work, a council-gate or integration-pass catch becomes mandatory before the next batch dispatches. Per-PR review remains necessary (it catches local defects) but insufficient (it cannot see the cross-PR state).
Source: docs/decisions.md (2026-07-14, "Structural bound and union defects"); milestones.md M-32 (council gate on M-30).
