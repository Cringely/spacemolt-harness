...
**Second drift mode on the same file pair: LENGTH, not content (issue #788).** Everything above is about *what* a guard reason says. This is about how much of it arrives. `src/planner/digest.ts:24` caps any quoted untrusted text at `UNTRUSTED_TEXT_SNIPPET_LEN = 200` characters, so a guard reason longer than that reaches the planner with its tail cut off. The tail is where a writer naturally puts the corrective detail. The credit-gift guard (`src/agent/executor.ts:1369-1392`) ran 229 characters with its roster of valid pilot names starting at 169. The names the planner needed were the part that got cut, so the one channel that could have taught it the right names delivered nothing for the entire life of #703. Both files' own tests passed throughout, because a substring matcher applied to the raw reason cannot see a clip that happens later.

Two things make this drift easy to reintroduce. The reason is assembled from several clauses, so appending one more, or reordering them, silently changes which clause falls off the end. And it embeds planner-authored text (the echoed gift target, unbounded by the registry), so the length is partly controlled by the very model the message is meant to correct. That is what `executor.ts:1386` defends against, clipping the echo to 24 characters, the ceiling `config.ts` puts on a real username.

**Spanning tests (new).** `test/deposit-gift-guard.test.ts:365` and `:395` run the real guard reason through the real `clipUntrusted` before matching, and import `UNTRUSTED_TEXT_SNIPPET_LEN` from `digest.ts` rather than hardcoding 200, so the two sides cannot drift apart silently. The second case uses an adversarially long target, which is what pins the echo clip; the first alone does not, because at exactly 24 characters the clip's comparison takes its other branch. `:436` pins the message's remaining budget, which is the part most likely to be spent by accident: with four maximum-length usernames the roster's last character ends at 197 of 200, so appending four characters to the guard's fixed text breaks it. Measured by running the guard, not computed. The deployed three-pilot fleet has 51 characters spare, and a fourth maximum-length pilot needs 6 more than exist whenever the target is also long. That ceiling is recorded in the test's comment rather than asserted, because an assertion on the overflow would turn red the day someone shortens the message. Note that `src` keeps no `executor.ts` → `digest.ts` import edge (digest already imports from executor, so that would cycle), which is precisely why this coupling has to be pinned by a test instead of by the type system.
...
**Spanning tests.** `test/agent-mission-progress.test.ts` ("NOT-ready verdict names the shortfall", "READY verdict names the complete_mission call", "count UNKNOWN → no verdict fabricated") pins side A. `test/executor.test.ts` ("objective short of required blocks before the wire", "every objective met passes through") pins side B. `test/improv-parity.test.ts` pins the guard↔§4-briefing half. A predicate change on one side that is not mirrored on the other fails at least one of these.

**Not part of this seam (#553).** Side B also refuses a `complete_mission` whose id is absent from the freshly parsed active list. That verdict has NO side-A mirror and is not supposed to get one. The digest renders at plan time and the guard re-reads at execution time. In the minutes between, an auto-assigned `Distress:` mission that the planner just read out of the listing is already gone when the step runs, and that is what the refusal catches. Mirroring it into the digest would state a membership fact that was true when written and is the very thing that goes stale. It is pinned on its own by `test/executor.test.ts` ("id absent from a parsed active list blocks before the wire (#553)") and by the `#553` entry in `test/improv-parity.test.ts`.
...
## 14. Dedup-key shape: work-order/brief prose ↔ `fileFinding`'s auto-normalize (#635)

**Side A**: `src/scheduler/spawn.ts`'s `FILING_HOWTO` (appended to every job's work order in `workOrder()`, `spawn.ts:151-180`, and joined into the composed prompt after the charter text in `composePrompt()`, `spawn.ts:182-194`) describes the dedup-key shape: condition-only, with severity/priority words left out, and names the auto-normalization behavior. `docs/briefs/council-review.md`'s Output section (its own `--dedup-key <stable-key>` line) describes the same command, without mentioning the auto-normalization at all. The two are the same command-form pairing entry 9 already tracks (spawn.ts and council-review.md both describing `file-finding.ts`'s flags), extended here to a second fact that only one of the two names.
**Side B**: `src/scheduler/filing.ts`'s `DEDUP_KEY_RE` (the charset a NEW key must satisfy) and `SEVERITY_WORDS`/`normalizeDedupKey` (the actual words stripped before `fileFinding`'s internal near-match check bumps instead of creating). A third filer-side constraint, `SUPPRESSION_NOTICE_KEY` (`filing.ts:431`), reserves one literal key for the filer's own standing suppression notice: `fileFinding` rejects it with a `FilingInputError` before any `gh` call (`filing.ts:739-741`), surfacing through `file-finding.ts` as exit 2 with `dedup-key is reserved by the filer`. No work-order or brief prose mentions it and none needs to; an agent that mints it is refused loudly rather than bumping a notice it does not own, so this is a guarded seam, not silent drift (spanning test: `test/scheduler-filing.test.ts`, "dedupKey === SUPPRESSION_NOTICE_KEY ⇒ rejected before any gh call").

**How they drift, and why it matters less than entry 9.** Unlike entry 9, correctness here does not depend on the prose. `fileFinding()` calls its own near-match check on every create, regardless of what any charter, brief, or work order says (PR #62 review, finding 1: the first cut of this fix put the whole invariant in a prose "search before you mint" step an agent could skip; this version does not). What does drift is the human/agent-facing description. If `SEVERITY_WORDS` gains or drops a word, both prose copies silently go stale, and `council-review.md` never described the behavior in the first place, so a reader of the brief alone would not know severity-word variants dedup on their own.

**Spanning tests.** `test/scheduler-filing.test.ts`'s "severity-word near-match auto-bump (#635 review finding 1)" block proves the actual behavior end-to-end (a severity-affix key variant bumps an OPEN issue, a different wording still creates two, a stated boundary rather than a silent gap) and needs no prose file to pass: this is what makes it rung-1. `test/scheduler-spawn.test.ts`'s "every work order names the dedup-key shape" only proves the guidance text reached the prompt, not that it is accurate. No automated check compares `SEVERITY_WORDS` against either prose copy's wording, or checks whether `council-review.md` mentions the auto-normalization at all, the same unchecked-prose gap entry 11 names for `guardrails.md`; a PR changing `SEVERITY_WORDS` should grep both prose files by hand.
