...
**Side B**: `src/scheduler/filing.ts`'s `DEDUP_KEY_RE` (the charset a NEW key must satisfy) and `SEVERITY_WORDS`/`normalizeDedupKey` (the actual words stripped before `fileFinding`'s internal near-match check bumps instead of creating).
