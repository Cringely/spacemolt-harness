...
**Last updated:** 2026-09-23, doc-steward pass reconciling cluster #137-#143 (tracker #1114/#1115/#1116/#1135 closed — the fleet-rescue blockers from 2026-09-22). Primary repo: github.com/Cringely/spacemolt-harness

## NOW, live status

_Refreshed 2026-09-23 (fifth pass, cluster #137-#143). Boot from this block + `docs/backlog.md` + `docs/game-reference/commands.md`._

**FIVE PRs MERGED, FOUR TRACKER ISSUES CLOSED — THE THREE CORSAIR-RESCUE FINDINGS PLUS THE DEDUPE CEREMONY.** #137 (`b53dffc`) is infrastructure (sync core-managed hooks from agent-harness-core, no issue cited). #139 (`045d765`) closes #1116: when the buy-price guard refuses a transaction, it now hands the planner a filled-in `create_buy_order` template (the #681 pattern) so the planner can re-steer without re-typing the details. #140 (`1f666a3`) closes #1115: a refuel give-up keyed on affordability now clears as soon as credits rise, instead of latching forever — the corsair was stranded when the PM gifted 1,500cr and nothing unstuck it. #142 (`af3e309`) closes #1114: the agent now surfaces which distress missions carry rescue requests, and logs when a request is rejected (no fleet rescue without human approval). #143 (`b985d59`) closes #1135: the weekly backlog dedupe ceremony shipped, gated off until the manual weekly pool reset (the spec from #132 is now live code).

**F4 UNBLOCKED — THE CORSAIR RESCUE'S REAL BOTTLENECK WAS HUMAN-GATED, NOT CODE.** The three issues from the 2026-09-20 rescue (#1114/#1115/#1116) are now fixed. Their root analysis: the pilot was stranded because three correct, independently-tested guards composed into a strand no single guard's test reaches (L-58). The fixes are producer-side guards at each seam (affordability latch, buy refusal, rescue request gating); the live incident proved all three necessary. F4's definition ("3 pilots/24h/zero strands, steer confirmed per pilot") requires human authorization for steer calls and fleet rescue, which the charter now requires. A capable defect that looked like a blocker turns out to have been a gating decision waiting for charter language.

**DEPLOY STATE, PM live capture (not verifiable from the repo).** Prod ran `7ab54a3` (PR #131's merge commit) from 2026-09-23T04:20:27Z through the corsair rescue. No image re-deploy confirmed since; the fixes in #139/#140/#142/#143 are docs/code/spec only; #143's ceremony is gated off until manual enable. Estimated deploy drift: all five PRs from this cluster should reach the host within ~1 minute of this merge via the auto-deploy cron.

**BACKLOG:** see `docs/backlog.md`, regenerated this pass.

**Next:** F4 is gate-ready (human authorization for steer/rescue is now charter-gated); the fleet can fly and has structural guards. #1133 (fix the dedupe filer itself, P1) remains. #1134 (two flaky scheduler tests, P2) is not blocking. Early-stage work on the dedupe ceremony (#1135 merged, awaiting manual enable per the charter).
