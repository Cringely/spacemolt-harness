...
**MERGED: PR #32 (#551 dock dead-end), 02:31Z, `81ee4d3`.** New `dockNoStationStreak` counter (incremented per `dock` refusal at `executeOne`) forces a reroute to the last known station after 3 consecutive refusals. Root cause: pilot retries `dock` without ever issuing a MOVE, so the existing `fuelBlockedMoves` strand guard never fires. Injects `[travel_to {last confirmed station}, dock]` using geography from #517/#525; fail-safe alerts if no station known and defers to strand/no-progress escalation. Incident (#551): 68 occurrences in 72h, largest failure class, preceded 8h stall + operator self_destruct at 2/130 fuel. Files: `agent.ts` (counter), `stall-monitor.ts` (guard logic), 7 new tests.
