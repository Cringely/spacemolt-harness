...
**Last updated:** 2026-09-21, doc-steward pass reconciling PR #127 (tracker #982/#1003 closed). Primary repo: github.com/Cringely/spacemolt-harness

## NOW, live status

_Refreshed 2026-09-21 (fourth pass, PR #127). Boot from this block + `docs/backlog.md` + `docs/game-reference/commands.md`._

**ONE PR MERGED, TWO TRACKER ISSUES CLOSED (LIVE INCIDENT CLASSES).** #127 (`6fd39e0`, merged 2026-09-21 03:38:37Z, two commits: `52d235b` initial fix + `703a7de` follow-up hardening) closes #982 (`sell{id:"wreck"}`, 8x/72h) and #1003 (`exotic_matter_sample` across sell/buy/deposit/withdraw, 7x/72h). Producer-side item-id catalog validation now gates plan admission for all seven item-bearing actions (buy/sell/jettison/withdraw/deposit/create_sell_order/create_buy_order), before any reach the executor. Reused the edit-distance suggestion pattern from `normalizePlanLocations` and consolidated `ITEM_PARAM_BY_ACTION` + `nearestCatalogItemId` from their two hand-maintained copies (eval/scorers.ts, executor.ts) into one shared SSOT in catalog.ts. Premise confirmed live: `wreck` and `exotic_matter_sample` absent from `src/catalog/catalog.data.json` (710 items). First commit (2017 pass, 1 pre-existing skip) + follow-up hardening (2024 pass, 1 pre-existing skip), typecheck clean; ablated four ways (guard logic, call-site wiring, shared map contents, improv-parity seam) with independent test failures. The follow-up fixed a regression where the item-retry reparsed plan but skipped location re-check, and bounded interpolated item id through `clipUntrusted` to prevent untrusted strings reaching LLM context. Added improv-mode §4 bullet (the improv driver has no plan-admission step to fall back on).

**DEPLOY STATE, NOT YET KNOWN.** Last confirmed prod image (from handoff, not this pass): `d7ab050` (2026-09-19 23:28:26Z, #123). Auto-deploy cron runs every 60s on the host, so `6fd39e0` reached production within ~1m of merge (2026-09-21 03:39-03:40Z) IF the cron fired in that window; health gate, rollback enabled. Unconfirmed: whether it stayed up, what the live signal looks like for #982/#1003 (no more `invalid_item` on wreck/exotic_matter_sample, or zero such calls if the guard prevents them entirely).

**BACKLOG:** see `docs/backlog.md`, regenerated this pass.

**Next:** watch the prod logs for zero further `wreck`/`exotic_matter_sample` incidents across the next 72h window (proving the incidents closed, not just the code deployed). Separately, F4 gate (3 pilots/24h/zero strands, steer confirmed per pilot) remains open. See L-57 on agent-vs-seat attribution for merged PRs.
