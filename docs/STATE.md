...
**BACKLOG MODEL (decided 2026-07-21).** The private `spacemolt` repo was UN-ARCHIVED to serve as the private ops/backlog tracker (SSOT for all 48 open work items + labels + history + cross-refs, kept intact by un-archiving in place). Public `spacemolt-harness` issues are a community-facing subset only (e.g. #7/#8/#9 pilot heartbeat/stall/routing fixes). So old numbers here (#183/#458/#459/#453/#213) are private-repo issues and stay valid. #458 P1 buy-guard (mine_resource counts MINED only; a prior titanium buy wasted ~120k cr; guard not yet landed). Slow SSH to the scheduler LXC (~30-40s connect; cause TBD, UseDNS already off). Codex review seat (#460): `bun scripts/codex-review.ts <PR>` advisory, run beside the Claude reviewer.
