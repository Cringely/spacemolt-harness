...
**Last updated:** 2026-09-23, doc-steward pass reconciling post-wave-4 cluster PRs #137, #140, #142. Primary repo: github.com/Cringely/spacemolt-harness

## NOW, live status

_Refreshed 2026-09-23 (fifth pass, cluster #137/#140/#142). Boot from this block + `docs/backlog.md` + `docs/game-reference/commands.md`._

**THREE PRs MERGED SINCE LAST PASS, TWO FIXES SHIPPED FOR #1114 AND #1115.** #137 (`b53dffc`, 2026-09-23T16:28:57Z) is a chore-only sync of four hook files from agent-harness-core, adds `.claude/.gitignore`, ships no pilot code. #140 (`1f666a3`, 2026-09-23T21:09:46Z) closes #1115: `reflexGaveUpAt` was a permanent latch keyed on (station, action) with no cause memory; a refuel give-up earned on insufficient credits never re-tested itself once credits changed. Now `classifyReflexFailureCause` identifies affordability failures, records the cause and credits balance at failure-time, and `reflexGaveUpAt` skips a matching record when current credits exceed the recorded ones — the latch self-invalidates without needing the exact refused price. Premise check notes #534 (the 9-hour corsair freeze) is a separate no-progress hold (`stall-monitor.ts`'s `progressFingerprint` already includes `credits`, so self-heals on credit change). #142 (`af3e309`, 2026-09-23T21:26:37Z) closes #1114: no pilot briefing had any fleet-mate's fuel or credits state, so a rescue could never occur to the planner even when miner held 50k+ while corsair sat at 0 fuel / 5 credits. Now `Agent.fleetDistress` reads other pilots' latest `status_snapshot` (written every wake, L5 already exists), flags fuel === 0 or credits below 20cr floor (the worst published station price at <10% tank, per `fuel.md`'s price table), and `digest.ts` renders the remedy in prose only, never a filled-in call (the #681/#1116 pattern). **Known gap for #1114: live proof (stranded pilot refuels with no human steer) needs #1115 merged and live — corsair's existing stranding still fails to consume a gift once one arrives.**

**DEPLOY STATE, PM live capture (not verifiable from the repo).** Prod ran `7ab54a3` (PR #131's merge commit) from 2026-09-23T04:20:27Z, confirmed healthy through 04:45Z, `RestartCount 0`. #132 (#135) is docs-only, no new image. #137 is chore-only, no new image. #140 and #142 merged after this session started (21:09:46Z, 21:26:37Z) — no live confirmation of either deployed yet. Assuming automatic image build/deploy on merge (previous pattern), `af3e309` (#142) may be live now, but without a capture timestamp this is expectation, not proof. The #1115 latch fix (#140) is the critical path blocker for #1114's live proof.

**BACKLOG:** see `docs/backlog.md`, regenerated this pass.

**Next:** (1) confirm #140 and #142 deploy and remain healthy in prod; (2) run live capture with the #1115 fix live to close #1114's own "done-when"; (3) #1133 (filer fix, P1) and #1135 (dedupe ceremony, P1) are process debt; (4) #1134 (two flaky scheduler tests, P2) is not blocking F4.
